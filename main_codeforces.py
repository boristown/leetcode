...
def solve(x1,y1,x2,y2):
    f1 = True
    nr,nc = len(ROW),len(COL)
    if not DROW[x1] or not DROW[x2]: f1 = False
    for i in K:
        xk = x1+i
        if xk>x2: break
        if not DROW[xk]:
            ...
            break
    if f1:
        px1 = ROW.bisect_left(x1)
        px2 = px1 + x2 - x1
        if px2 >= nr or ROW[px2]!=x2: f1 = False
    if f1:
        return "Yes"
    if not DCOL[y1] or not DCOL[y2]: return "No"
    for i in K:
        yk = y1+i
        if yk>y2: break
        if not DCOL[yk]:
            return "No"
    py1 = COL.bisect_left(y1)
    py2 = py1 + y2 - y1
    if py2 >= nc or COL[py2]!=y2: return "No"
    return "Yes"
...
DCOL = Counter()
DROW = Counter()
n,q = map(int,input().split()) #input tuple
